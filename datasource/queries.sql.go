...
VALUES ($1, $2, $3) RETURNING id, userid, task, done
`
...
const findTodoByUser = `-- name: FindTodoByUser :many
SELECT id, userid, task, done FROM todos
WHERE userId = $1
`

func (q *Queries) FindTodoByUser(ctx context.Context, userid int32) ([]Todo, error) {
	rows, err := q.db.QueryContext(ctx, findTodoByUser, userid)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
